...
export const createHttpClient = async function (baseAddress: string): Promise<AxiosInstance> {
  ...
  const config: AxiosRequestConfig = {
    baseURL: baseAddress === '/proxy' ? '' : baseAddress
  }
...
export const createElsaClient = async function (serverUrl: string): Promise<ElsaClient> {
  if (_elsaClient) return _elsaClient
  ...
  _elsaClient = {
    ...
    workflowDefinitionsApi: {
      list: async (page?: number, pageSize?: number, versionOptions?: VersionOptions, searchTerm?: string) => {
        ...
        const response = await httpClient.get<PagedList<WorkflowDefinitionSummary>>(
          `v1/workflow-definitions${queryStringText}`
        )
      ...
      getMany: async (ids: Array<string>, versionOptions?: VersionOptions) => {
        ...
        const response = await httpClient.get<ListModel<WorkflowDefinitionSummary>>(
          `v1/workflow-definitions?ids=${ids.join(',')}&version=${versionOptionsString}`
        )
        return response.data.items
      },
      getVersionHistory: async (definitionId: string): Promise<Array<WorkflowDefinitionVersion>> => {
        const response = await httpClient.get<ListModel<WorkflowDefinitionVersion>>(
      ...
      getByDefinitionAndVersion: async (definitionId: string, versionOptions: VersionOptions) => {
        const versionOptionsString = getVersionOptionsString(versionOptions)
        const response = await httpClient.get<WorkflowDefinition>(
          `v1/workflow-definitions/${definitionId}/${versionOptionsString}`
        )
        return response.data
      },
      save: async (request) => {
      ...
      retract: async (workflowDefinitionId) => {
        const response = await httpClient.post<WorkflowDefinition>(
      ...
      revert: async (workflowDefinitionId, version) => {
        const response = await httpClient.post<WorkflowDefinition>(
          `v1/workflow-definitions/${workflowDefinitionId}/revert/${version}`
        )
        return response.data
      },
      export: async (workflowDefinitionId, versionOptions): Promise<ExportWorkflowResponse> => {
        ...
        const response = await httpClient.post(
          `v1/workflow-definitions/${workflowDefinitionId}/${versionOptionsString}/export`,
          null,
          {
            responseType: 'blob'
          }
        ...
        const fileName = contentDispositionHeader
          ...
          : `workflow-definition-${workflowDefinitionId}.json`
    ...
    workflowRegistryApi: {
      list: async (
      ...
      ): Promise<PagedList<WorkflowBlueprintSummary>> => {
        const queryString = {
          version: getVersionOptionsString(versionOptions)
        }
        ...
        const queryStringItems = collection.map(queryString, (v, k) => `${k}=${v}`)
        const queryStringText = queryStringItems.length > 0 ? `?${queryStringItems.join('&')}` : ''
        const response = await httpClient.get<PagedList<WorkflowBlueprintSummary>>(
          `v1/workflow-registry/by-provider/${providerName}${queryStringText}`
        )
        return response.data
      },
      ...
      findManyByDefinitionVersionIds: async (
        definitionVersionIds: Array<string>
      ): Promise<Array<WorkflowBlueprintSummary>> => {
        if (definitionVersionIds.length == 0) return []

        const idsQuery = definitionVersionIds.join(',')
        const response = await httpClient.get<Array<WorkflowBlueprintSummary>>(
          `v1/workflow-registry/by-definition-version-ids?ids=${idsQuery}`
        )
        return response.data
      },
    ...
    workflowInstancesApi: {
      list: async (
        ...
        const queryStringItems = collection.map(queryString, (v, k) => `${k}=${v}`)
        const queryStringText = queryStringItems.length > 0 ? `?${queryStringItems.join('&')}` : ''
        const response = await httpClient.get<PagedList<WorkflowInstanceSummary>>(
          `v1/workflow-instances${queryStringText}`
        )
        return response.data
      },
      get: async (id) => {
    ...
    workflowExecutionLogApi: {
      get: async (
        ...
        page?: number,
        pageSize?: number
      ): Promise<PagedList<WorkflowExecutionLogRecord>> => {
        const queryString = {}
        ...
        const response = await httpClient.get(
          `v1/workflow-instances/${workflowInstanceId}/execution-log${queryStringText}`
        )
        return response.data
      }
    },
    scriptingApi: {
      getJavaScriptTypeDefinitions: async (
        workflowDefinitionId: string,
        context?: IntellisenseContext
      ): Promise<string> => {
        const response = await httpClient.post<string>(
          `v1/scripting/javascript/type-definitions/${workflowDefinitionId}?t=${new Date().getTime()}`,
          context
        )
        return response.data
      }
...
export interface WorkflowDefinitionsApi {
  list(
    ...
    versionOptions?: VersionOptions,
    searchTerm?: string
  ): Promise<PagedList<WorkflowDefinitionSummary>>
...
export interface CustomApi {
  workflowDefinitionsApi: {
    getByDefinitionId: AxiosRequestConfig
    save: AxiosRequestConfig
  }
  activitiesApi: {
