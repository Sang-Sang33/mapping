...
export class ElsaWorkflowDefinitionEditorScreen {
  ...
  @Prop({ attribute: 'custom-workflow-definitions-api', reflect: true })
  ...
  @Event({ bubbles: false }) initializing: EventEmitter
  ...
  customGetWorkflowDefintion = async (workflowDefinitionId: string) => {
    const httpClient = await createHttpClient(this.serverUrl)
    const getByDefinitionId = this.customApi?.workflowDefinitionsApi?.getByDefinitionId
    if (getByDefinitionId) {
      const { method, url } = getByDefinitionId
      const workflowDefinitionList = await httpClient({
        url: url + '?' + `ids=${workflowDefinitionId}`,
        method
      }).then((res) => res.data)
      return workflowDefinitionList[0]
    } else {
      console.error('no custom "getByDefinitionId" api, request error')
    }
  }
  ...
  async workflowDefinitionIdChangedHandler(newValue: string) {
    ...
    if (workflowDefinitionId && workflowDefinitionId.length > 0) {
      try {
        if (this.isCustomApi) {
          workflowDefinition = await this.customGetWorkflowDefintion(workflowDefinitionId)
        } else {
          workflowDefinition = await client.workflowDefinitionsApi.getByDefinitionAndVersion(workflowDefinitionId, {
            isLatest: true
          })
        }
      } catch {
  ...
  async loadWorkflowDefinitions() {
    ...
    return (await elsaClient.workflowDefinitionsApi.list(page, pageSize, latestOrPublishedVersionOptions)).items
  }

  async getRunWorkflowActivityDescriptors() {
    const workflowDefinitions = await this.loadWorkflowDefinitions()
    const runWorkflowDescriptors: ActivityDescriptor[] = workflowDefinitions.map((x) => ({
      type: 'RunWorkflow', // 控制活动显示图标
  ...
  async saveWorkflowInternal(workflowModel?: WorkflowModel, publish?: boolean) {
    ...
    const isNew =
      typeof workflowDefinition.definitionId === 'undefined' && typeof this.workflowDefinitionId === 'undefined'
    ...
    try {
      ...
      console.debug('Old workflow definition...', this.workflowDefinition)

      const save = this.customApi?.workflowDefinitionsApi?.save
      if (this.isCustomApi && save) {
        const { method, url } = this.customApi.workflowDefinitionsApi.save
        await httpClient({
          method,
          url,
          data: request
        }).then((res) => res.data)
        workflowDefinition = await this.customGetWorkflowDefintion(request.workflowDefinitionId)
      } else {
        workflowDefinition = await client.workflowDefinitionsApi.save(request)
      }
      console.debug('Latest workflow definition...', workflowDefinition)
  ...
  async onDeleteClicked() {
    ...
    const result = await this.confirmDialog.show(
      t('DeleteConfirmationModel.Title'),
      t('DeleteConfirmationModel.Message')
    )
  ...
  onTestActivityMessageReceived = async (args) => {
    ...
    if (message) {
      ...
      this.workflowTestActivityMessages = this.workflowTestActivityMessages.filter(
        (x) => x.activityId !== message.activityId
      )
  ...
  private clearSubsequentWorkflowTestMessages(activityId: string) {
    const targetActivityId = this.workflowDefinition.connections.find(
      (x) => x.sourceActivityId === activityId
    )?.targetActivityId

    if (!targetActivityId) return

    this.workflowTestActivityMessages = this.workflowTestActivityMessages.filter(
      (x) => x.activityId !== targetActivityId || x.status === WorkflowTestActivityMessageStatus.Failed
    )
      ...
      <div class="elsa-flex-1 elsa-flex elsa-relative">
        ...
        {state.useX6Graphs && (
          <x6-designer
            ...
            layoutDirection={this.layoutDirection}
            activityContextMenuButton={
              this.workflowDesignerMode == WorkflowDesignerMode.Edit ? () => '' : this.renderActivityStatsButton
            }
            onActivityContextMenuButtonClicked={(e) => this.onActivityContextMenuButtonClicked(e)}
            onActivityContextMenuButtonTestClicked={(e) => this.onActivityContextMenuButtonTestClicked(e)}
            activityContextMenu={
  ...
  private canBeRestartedFromCurrentActivity() {
    ...
    const sourceActivityId = this.workflowDefinition.connections.find(
      (x) => x.targetActivityId === this.selectedActivityId
    )?.sourceActivityId

    return (
      sourceActivityId &&
      this.workflowTestActivityMessages.some(
        (x) => x.activityId === sourceActivityId && x.status !== WorkflowTestActivityMessageStatus.Failed
      )
    )
  }
  ...
  renderTestActivityMenu = () => {
        ...
        <div class="elsa-relative elsa-grid elsa-gap-6 elsa-bg-white px-5 elsa-py-6 sm:elsa-gap-8 sm:elsa-p-8">
          ...
          {collection.map(filteredData, (v, k) => (
            <div class="elsa-ml-4">
              ...
              <pre
                class="elsa-mt-1 elsa-text-sm elsa-text-gray-500 elsa-overflow-x-auto"
    ...
    return (
      <div
        ...
        class={`${
          this.activityContextMenuTestState.shown ? '' : 'hidden'
        } elsa-absolute elsa-z-10 elsa-mt-3 elsa-px-2 elsa-w-screen elsa-max-w-xl sm:elsa-px-0`}
        style={{
      ...
      >
        <div
          class="elsa-rounded-lg elsa-shadow-lg elsa-ring-1 elsa-ring-black elsa-ring-opacity-5 elsa-overflow-x-hidden elsa-overflow-y-auto"
  ...
  renderActivityContextMenu() {
    ...
    return (
      <div
        ...
        data-transition-leave-end="elsa-transform elsa-opacity-0 elsa-scale-95"
        class={`${
          this.activityContextMenuState.shown ? '' : 'hidden'
        } context-menu elsa-z-20 elsa-mx-3 elsa-w-48 elsa-mt-1 elsa-rounded-md elsa-shadow-lg elsa-fixed`}
        style={{ left: `${this.activityContextMenuState.x}px`, top: `${this.activityContextMenuState.y}px` }}
        ref={(el) => (this.activityContextMenu = el)}
      >
        <div
          class="elsa-rounded-md elsa-bg-white elsa-shadow-xs"
          role="menu"
        ...
        >
          ...
          <div class="elsa-py-1">
            <a
              ...
              role="menuitem"
            >
              {selectedActivities.length > 1 && selectedActivities.indexOf(activity.activityId) !== -1
                ? t('ActivityContextMenu.DeleteSelected')
  ...
  renderActivityPicker() {
    return <elsa-activity-picker-modal serverUrl={this.serverUrl} />
  }

  renderActivityDragPanel() {
    return <elsa-activity-drag-panel serverUrl={this.serverUrl}></elsa-activity-drag-panel>
  }
        ...
        <elsa-modal-dialog ref={(el) => (this.helpDialog = el)}>
          {!state.useX6Graphs && (
            <div slot="content" class="elsa-p-8">
              <h3 class="elsa-text-lg elsa-font-medium">{this.t('HelpDialog.Title')}</h3>
              <dl class="elsa-mt-2 elsa-border-t elsa-border-b elsa-border-gray-200 elsa-divide-y elsa-divide-gray-200">
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500">{this.t('HelpDialog.DeleteConnections.Label')}</dt>
                  <dd class="elsa-text-gray-900">{this.t('HelpDialog.DeleteConnections.Description')}</dd>
                </div>
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500">{this.t('HelpDialog.ConnectOutcomesToExistingActivity.Label')}</dt>
                  <dd class="elsa-text-gray-900">
                    {this.t('HelpDialog.ConnectOutcomesToExistingActivity.Description')}
                  </dd>
                </div>
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500"> {this.t('HelpDialog.Pan.Label')}</dt>
                  <dd class="elsa-text-gray-900">{this.t('HelpDialog.Pan.Description')}</dd>
                </div>
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500">{this.t('HelpDialog.Zoom.Label')}</dt>
                  <dd class="elsa-text-gray-900">{this.t('HelpDialog.Zoom.Description')}</dd>
                </div>
              </dl>
            </div>
          )}
          {state.useX6Graphs && (
            <div slot="content" class="elsa-p-8">
              <h3 class="elsa-text-lg elsa-font-medium">{this.t('X6GraphsHelpDialog.Title')}</h3>
              <dl class="elsa-mt-2 elsa-border-t elsa-border-b elsa-border-gray-200 elsa-divide-y elsa-divide-gray-200">
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500">{this.t('X6GraphsHelpDialog.DeleteConnections.Label')}</dt>
                  <dd class="elsa-text-gray-900">{this.t('X6GraphsHelpDialog.DeleteConnections.Description')}</dd>
                </div>
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500">{this.t('X6GraphsHelpDialog.EditActivities.Label')}</dt>
                  <dd class="elsa-text-gray-900">{this.t('X6GraphsHelpDialog.EditActivities.Description')}</dd>
                </div>
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500">{this.t('X6GraphsHelpDialog.Pan.Label')}</dt>
                  <dd class="elsa-text-gray-900">{this.t('X6GraphsHelpDialog.Pan.Description')}</dd>
                </div>
                <div class="elsa-py-3 elsa-flex elsa-justify-between elsa-text-sm elsa-font-medium">
                  <dt class="elsa-text-gray-500">{this.t('X6GraphsHelpDialog.Zoom.Label')}</dt>
                  <dd class="elsa-text-gray-900">{this.t('X6GraphsHelpDialog.Zoom.Description')}</dd>
                </div>
              </dl>
            </div>
          )}
    ...
    return (
      <elsa-flyout-panel expandButtonPosition={3}>
        <elsa-tab-header tab="general" slot="header">
          {this.t('WorkflowPanel.Tab.General')}
        </elsa-tab-header>
    ...
    if (isFeaturePanelVisible) {
      return [
        <elsa-tab-header tab="designer" slot="header">
          {this.t('WorkflowPanel.Tab.Designer')}
        </elsa-tab-header>,
  ...
  private renderVersionHistoryPanel = (workflowDefinition: WorkflowDefinition) => {
    return [
      <elsa-tab-header tab="versionHistory" slot="header">
        {this.t('WorkflowPanel.Tab.VersionHistory')}
      </elsa-tab-header>,
  ...
  onVersionSelected = async (e: CustomEvent<WorkflowDefinitionVersion>) => {
    ...
    const workflowDefinition = await client.workflowDefinitionsApi.getByDefinitionAndVersion(version.definitionId, {
...
DashboardTunnel.injectProps(ElsaWorkflowDefinitionEditorScreen, [
  'serverUrl',
  'culture',
  'monacoLibPath',
  'basePath',
