...
export class ElsaWorkflowPublishButton {
  ...
  onWindowClicked(event: Event) {
    ...
    if (!this.element.contains(target)) this.closeMenu()
  }
      ...
      <Host class="elsa-block" ref={(el) => (this.element = el)}>
        <span class="elsa-relative elsa-z-0 elsa-inline-flex elsa-shadow-sm elsa-rounded-md">
          ...
          <span class="-elsa-ml-px elsa-relative elsa-block">
            ...
            >
              <div
                class="elsa-divide-y elsa-divide-gray-100 focus:elsa-outline-none"
                role="menu"
              ...
              >
                <div class="elsa-py-1" role="none">
                  <a
                    href="#"
                    onClick={(e) => this.onExportClick(e)}
                    class="elsa-block elsa-px-4 elsa-py-2 elsa-text-sm elsa-text-gray-700 hover:elsa-bg-gray-100 hover:elsa-text-gray-900"
                    role="menuitem"
                  >
                    {t('Export')}
                  </a>
                  ...
                  <a
                    href="#"
                    onClick={(e) => this.onImportClick(e)}
                    class="elsa-block elsa-px-4 elsa-py-2 elsa-text-sm elsa-text-gray-700 hover:elsa-bg-gray-100 hover:elsa-text-gray-900"
                    role="menuitem"
                  >
                    {t('Import')}
                  ...
                  <a
                    ...
                    class="elsa-block elsa-px-4 elsa-py-2 elsa-text-sm elsa-text-red-700 hover:elsa-bg-gray-100 hover:elsa-text-gray-900"
                    role="menuitem"
                  >
                    {t('Delete')}
                  </a>
  ...
  renderMainButton = () => {
    ...
    if (isPublished) return this.publishing ? this.renderUnpublishingButton() : this.renderUnpublishButton()
    else return this.publishing ? this.renderPublishingButton() : this.renderPublishButton()
  }
  ...
  renderPublishButton = () => {
    ...
    if (isLatest) return this.renderButton(this.t('Publish'), this.onPublishClick)
  ...
  renderPublishingButton() {
    ...
    const isLatest = workflowDefinition.isLatest
    const version = workflowDefinition.version
    const text = isLatest ? this.t('Publishing') : `Publishing version ${version}`

    return this.renderLoadingButton(text)
  }

  renderUnpublishButton = () => {
    return this.renderButton(this.t('Unpublish'), (e) => this.onUnPublishClick(e))
  }
  ...
  renderMenuItem = (text: string, handler: (e: Event) => any) => {
    if (!this.workflowDefinition.isPublished) return undefined
  ...
  renderLoadingButton(text: string) {
    ...
    return (
      ...
      >
        <svg
          class="elsa-animate-spin -elsa-ml-1 elsa-mr-3 elsa-h-5 elsa-w-5 elsa-text-blue-400"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
        >
