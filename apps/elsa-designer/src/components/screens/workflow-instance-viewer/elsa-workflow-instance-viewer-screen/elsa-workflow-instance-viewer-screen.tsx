...
export class ElsaWorkflowInstanceViewerScreen {
  ...
  async serverUrlChangedHandler(newValue: string) {
    if (newValue && newValue.length > 0) await this.loadActivityDescriptors()
  }
  ...
  onWindowClicked(event: Event) {
    ...
    if (!this.contextMenu.contains(target)) this.handleContextMenuChange(0, 0, false, null)
  }
  ...
  mapWorkflowModel(workflowBlueprint: WorkflowBlueprint, workflowInstance: WorkflowInstance): WorkflowModel {
    const activities = workflowBlueprint.activities
      ...
      .map((x) => this.mapActivityModel(x, workflowInstance))
    const connections = workflowBlueprint.connections
      .filter(
        (c) =>
          activities.findIndex((a) => a.activityId == c.sourceActivityId || a.activityId == c.targetActivityId) > -1
      )
          ...
          <div>
            <table class="elsa-min-w-full elsa-divide-y elsa-divide-gray-200 elsa-border-b elsa-border-gray-200">
              ...
              <tbody class="elsa-bg-white elsa-divide-y elsa-divide-gray-200">
                {activityStats.eventCounts.length > 0 ? (
