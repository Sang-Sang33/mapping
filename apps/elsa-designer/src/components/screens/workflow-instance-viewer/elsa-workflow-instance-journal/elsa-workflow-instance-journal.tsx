...
export class ElsaWorkflowInstanceJournal {
  ...
  async workflowInstanceIdChangedHandler(newValue: string) {
    ...
    const getWorkflowInstanceExecutionLogById = this.customApi?.workflowInstanceApi?.getWorkflowInstanceExecutionLogById

    if (workflowInstanceId && workflowInstanceId.length > 0) {
      try {
        if (this.isCustomApi && getWorkflowInstanceExecutionLogById) {
          const { url } = getWorkflowInstanceExecutionLogById
          this.records = await httpClient.get(url + '/' + newValue + '/execution-log').then((res) => res.data)
        } else {
  ...
  selectActivityRecordInternal(record?: WorkflowExecutionLogRecord) {
    ...
    this.selectedActivityId =
      activity != null
        ? !!activity.parentId && activity.parentId != this.workflowBlueprint.id
          ? activity.parentId
          : activity.id
        : null
  }
  ...
  renderJournalTab = () => {
    ...
    const renderRecord = (record: WorkflowExecutionLogRecord, index: number) => {
      const prevItemReverseIndex = allItems
      ...
      const activityName =
        activityBlueprint.displayName ||
        activityBlueprint.name ||
        activityDescriptor.displayName ||
        activityDescriptor.type ||
        '(Not Found): ' + activityType
      const eventName = record.eventName
      const eventColor = this.getEventColor(eventName)
      const recordClass =
        record.id === selectedRecordId ? 'elsa-border-blue-600' : 'hover:elsa-bg-gray-100 elsa-border-transparent'
        ...
        return (
          <div>
            ...
            {exception.InnerException ? (
              <div class="elsa-ml-4">{renderExceptionMessage(exception.InnerException)}</div>
            ) : undefined}
          </div>
        )
      }
                  ...
                  <div>
                    <span class="elsa-relative elsa-inline-flex elsa-items-center elsa-rounded-full elsa-border elsa-border-gray-300 elsa-px-3 elsa-py-0.5 elsa-text-sm">
                      <span class="elsa-absolute elsa-flex-shrink-0 elsa-flex elsa-items-center elsa-justify-center">
                        <span class={`elsa-h-1.5 elsa-w-1.5 elsa-rounded-full ${eventColor}`} aria-hidden="true" />
                      </span>
  ...
  renderActivityStateTab = () => {
    const activityModel =
      !!this.workflowModel && this.selectedActivityId
          ...
          <dd class="elsa-text-gray-900 elsa-break-all">
            <span class="elsa-relative elsa-inline-flex elsa-items-center elsa-rounded-full">
              <span class="elsa-flex-shrink-0 elsa-flex elsa-items-center elsa-justify-center">
                <span class={`elsa-w-2-5 elsa-h-2-5 elsa-rounded-full ${eventColor}`} aria-hidden="true" />
              </span>
