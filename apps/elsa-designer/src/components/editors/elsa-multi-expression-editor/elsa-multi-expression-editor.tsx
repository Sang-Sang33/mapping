...
export class ElsaMultiExpressionEditor {
  ...
  @Prop({ attribute: 'editor-height', reflect: true }) editorHeight = '10em'
  ...
  onWindowClicked(event: Event) {
    ...
    if (!this.contextMenuWidget || !this.contextMenuWidget.contains(target)) this.closeContextMenu()
  }
  ...
  closeContextMenu() {
    if (this.contextMenu) leave(this.contextMenu)
  }
  ...
  render() {
    return (
      <div>
        <div class="elsa-mb-1">
          <div class="elsa-flex">
            <div class="elsa-flex-1">{this.renderLabel()}</div>
  ...
  renderContextMenuWidget() {
    if (this.supportedSyntaxes.length == 0) return undefined

    const selectedSyntax = this.selectedSyntax
    const advancedButtonClass = selectedSyntax ? 'elsa-text-blue-500' : 'elsa-text-gray-300'

    return (
      <div class="elsa-relative" ref={(el) => (this.contextMenuWidget = el)}>
        <button
          type="button"
          class={`elsa-border-0 focus:elsa-outline-none elsa-text-sm ${advancedButtonClass}`}
        ...
        <div>
          <div
            ...
            class="hidden elsa-origin-top-right elsa-absolute elsa-right-1 elsa-mt-1 elsa-w-56 elsa-rounded-md elsa-shadow-lg elsa-bg-white elsa-ring-1 elsa-ring-black elsa-ring-opacity-5 elsa-divide-y elsa-divide-gray-100 focus:elsa-outline-none elsa-z-10"
            role="menu"
          ...
          >
            <div class="elsa-py-1" role="none">
              <a
                onClick={(e) => this.selectSyntax(e, null)}
                href="#"
                class={`elsa-block elsa-px-4 elsa-py-2 elsa-text-sm hover:elsa-bg-gray-100 hover:elsa-text-gray-900 ${
                  !selectedSyntax ? 'elsa-text-blue-700' : 'elsa-text-gray-700'
                }`}
                role="menuitem"
              >
                Default
            ...
            <div class="elsa-py-1" role="none">
              {this.supportedSyntaxes.map((syntax) => (
                <a
                  onClick={(e) => this.selectSyntax(e, syntax)}
                  href="#"
                  class={`elsa-block elsa-px-4 elsa-py-2 elsa-text-sm hover:elsa-bg-gray-100 hover:elsa-text-gray-900 ${
                    selectedSyntax == syntax ? 'elsa-text-blue-700' : 'elsa-text-gray-700'
                  }`}
                  role="menuitem"
                >
                  {syntax}
  ...
  renderContextMenuButton() {
    if (!this.selectedSyntax)
      return (
        <svg
          class="elsa-h-5 elsa-w-5 elsa-text-gray-400"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          stroke-width="2"
      ...
      <div>
        <div class={expressionEditorClass}>
          <elsa-expression-editor
            ref={(el) => (this.expressionEditor = el)}
