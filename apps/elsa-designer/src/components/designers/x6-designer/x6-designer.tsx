...
export class ElsaWorkflowDesigner {
  ...
  ignoreCopyPasteActivities = false
  silent = false
  ignoreNextNodeSelect = false
  ...
  @Prop() canvasHeight?: string = 'calc(100vh - 64px)'
  @Event({
  ...
  applyAutoLayout() {
    ...
    this.workflowModel.activities.forEach((activity) => {
      const activityElement = document
        .querySelectorAll('g[data-cell-id="' + activity.activityId + '"]')[0]
  ...
  getOutcomes = (activity: ActivityModel, definition: ActivityDescriptor): Array<string> => {
    ...
    if (definition) {
      ...
      if (lambda instanceof Array) {
        ...
        if (value instanceof Array) outcomes = value
  ...
  private onEdgeConnected = async (e: { edge: Edge }) => {
    ...
    if (!this.enableMultipleConnectionsFromSingleSource) {
      const existingConnectionIndex = workflowModel.connections.findIndex(
        (x) => x.sourceId === sourceNode.id && x.targetId == targetId
      )
      ...
      if (existingConnectionIndex === -1) {
        ...
        edge.data = newConnection
        edge.setLabels(sourcePort)

        workflowModel.connections = [...workflowModel.connections]
        workflowModel.connections[existingConnectionIndex] = newConnection
        this.updateWorkflowModel(workflowModel)
      }
    }
  }
  ...
  private createGraphNode = (item: ActivityModel): Node.Metadata => {
    ...
    const outcomes: Array<string> =
      this.mode === WorkflowDesignerMode.Blueprint ? item.outcomes : this.getOutcomes(item, descriptor)
    const ports = [{ group: 'in', id: uuid(), attrs: {} }]
  ...
  private getEdgeStep = (sourceId: string, targetId: string) => {
    ...
    const siblingActivities = siblingEdges
      .map((x) => this.workflowModel.activities.find((a) => a.activityId === x.targetId))
      .filter((x) => !!x)
  ...
  private createEdge = (connection: ConnectionModel, nodes?: Node.Metadata[]): Edge.Metadata => {
    let targetPort = connection.targetPort
    if (nodes?.length) {
      const findNode = nodes.find((n) => n.id === connection.targetId)
      targetPort = (findNode.ports as PortManager.Metadata).items.find((x) => x.group === 'in')?.id
    }
    ...
    return {
      ...
      target: { cell: connection.targetId, port: targetPort },
      targetPort,
  ...
  async getActivityDisplayContext(activityModel: ActivityModel): Promise<ActivityDesignDisplayContext> {
    ...
    if (!descriptor) descriptor = this.createNotFoundActivityDescriptor(activityModel)
  ...
  private updateModelFromGraph = (e?: { edge: Edge; node: Node }) => {
    ...
    const connections = graphModel.cells
      ...
      .filter((x) => x.shape == 'elsa-edge' && !!x.data)
      .map((x) => x.data as ConnectionModel)
  ...
  private updateGraph = async () => {
    ...
    for (const connection of connections) {
      const edge: Edge.Metadata = this.createEdge(connection, nodes)
      edges.push(edge)
    }
  ...
  private findActivityById = (id: string): ActivityModel =>
    this.workflowModel.activities.find((x) => x.activityId === id)
  ...
  updateWorkflowModel(model: WorkflowModel, emitEvent = true) {
  ...
  newActivity(activityDescriptor: ActivityDescriptor, point?: Point): ActivityModel {
    const graphRect = this.el.getBoundingClientRect()
    point = point ?? this.graph.pageToLocal(graphRect.width / 2, graphRect.height / 2)
    const SubCategory = activityDescriptor.customAttributes.SubCategory

    const displayName = SubCategory
      ? SubCategory + '.' + activityDescriptor.displayName
    ...
    // const WorkflowDefinitionId = activityDescriptor.customAttributes.WorkflowDefinitionId
  ...
  async addActivity(activity: ActivityModel, sourceActivityId?: string, targetActivityId?: string, outcome?: string) {
    ...
    if (targetActivityId) {
      const existingConnection = workflowModel.connections.find(
        (x) => x.targetId == targetActivityId && x.outcome == outcome
      )
    ...
    if (sourceActivityId != null) {
      const existingConnection = workflowModel.connections.find(
        (x) => x.sourceId == sourceActivityId && x.outcome == outcome
      )
    ...
    const newNode = this.createGraphNode(activity)
  ...
  addConnection(sourceActivityId: string, targetActivityId: string, outcome: string): ConnectionModel {
    ...
    if (!this.enableMultipleConnectionsFromSingleSource)
      connections = [
        ...workflowModel.connections.filter((x) => !(x.sourceId === sourceActivityId && x.outcome === outcome))
      ]
  ...
  async updateActivityExternal(activity: ActivityModel) {
    const originalActivity = this.findActivityById(activity.activityId)
  ...
  async showActivityPicker() {
    ...
    await eventBus.emit(EventTypes.ShowActivityDragPanel)
  }
  ...
  onActivityPicked = async (args) => {
    ...
    await this.showActivityEditor(activityModel, true)
  ...
  onActivityDrop = async (e: DragEvent) => {
    this.addingActivity = true
    const { pageX, pageY } = e
    const { offsetX, offsetY } = JSON.parse(e.dataTransfer.getData('position'))
    const activityDescriptor: ActivityDescriptor = JSON.parse(e.dataTransfer.getData('activityDescriptor'))
    const point = this.graph.pageToLocal(pageX - offsetX, pageY - offsetY)
    const activityModel = this.newActivity(activityDescriptor, point)
    await this.showActivityEditor(activityModel, true)
    // const connectFromRoot = !this.parentActivityOutcome || this.parentActivityOutcome == ''
    // const sourceId = connectFromRoot ? null : this.parentActivityId
    // const targetId = connectFromRoot ? this.parentActivityId : null
  ...
  onActivityDragOver = (e: DragEvent) => {
    e.preventDefault()
  }
    ...
    if (this.mode === WorkflowDesignerMode.Test) {
      setTimeout(() => {
        ...
        nodeEl?.addEventListener('click', (evt: MouseEvent) => {
          ...
          evt.stopPropagation()
          this.activityContextMenuButtonTestClicked.emit({
            x: evt.clientX,
            y: evt.clientY,
            shown: true,
            activity: activity
  ...
  renderActivity(activity: ActivityModel) {
    ...
    const context = { activityIcon, activityDescriptor: descriptor }
    ...
    return `<div class="elsa-border-2 elsa-border-solid ${cssClass}">
      <div class="elsa-p-2" style="padding-right: 2.6rem">
        <div class="elsa-flex elsa-justify-between elsa-space-x-4 mr-4">
          ...
          <div class="context-menu-button-container elsa-absolute elsa-z-1" stats-activity-id="${
            activity.activityId
          }" style="right: 0.5rem">
    ...
    return (
      ...
576-4.352-6.0416-8.96-11.6736-13.568-17.3568l-1.6384-2.048a306.688 306.688 0 0 0-115.9168-86.1696l-3.6352-1.6384a368.128 368.128 0 0 0-21.8624-7.936c-2.6624-0.8704-5.2736-1.8432-7.9872-2.56a301.8752 301.8752 0 0 0-19.456-5.0176c-3.7376-0.8704-7.3216-1.7408-10.9568-2.4576-1.8432-0.3584-3.584-0.8704-5.376-1.2288-4.864-0.8192-9.7792-1.1776-14.6944-1.8432-3.4304-0.4096-6.7584-0.9216-10.1888-1.28a331.2128 331.2128 0 0 0-24.576-1.2288c-1.536 0-2.9696-0.2048-4.4544-0.2048l-0.8192 0.0512a310.0672 310.0672 0 0 0-177.152 55.5008 36.608 36.608 0 0 0-9.216 51.3024 37.4784 37.4784 0 0 0 51.9168 9.0624 235.6736 235.6736 0 0 1 136.96-42.0864c7.0656 0 13.9776 0.3584 20.736 1.024l6.2976 0.768c5.632 0.6656 11.1616 1.536 16.64 2.56 2.4064 0.4608 4.8128 1.0752 7.168 1.5872 5.376 1.2288 10.752 2.56 16.0256 4.096l4.9152 1.6384c5.9904 1.8944 11.8784 4.0448 17.7152 6.4l1.7408 0.768a237.056 237.056 0 0 1 88.9344 65.9968l0.4096 0.5632a230.8096 230.8096 0 0 1 53.6064 147.8656h-54.6816a3.9936 3.9936 0 0 0-3.5328 2.048 3.7376 3.7376 0 0 0 0.2048 3.9424l91.9552 136.4992a3.9424 3.9424 0 0 0 3.328 1.7408 3.9936 3.9936 0 0 0 3.2768-1.7408l91.9552-136.4992a3.7888 3.7888 0 0 0 0.1536-3.9424 3.9936 3.9936 0 0 0-3.5328-2.048h-54.6304z"
            p-id="7743"
          ></path>
        </svg>
      </button>
  ...
  private renderAddActivityButton() {
    return (
      <button
        type="button"
        class="activity-add-button elsa-fixed elsa-top-20 elsa-inline-flex elsa-items-center elsa-p-2 elsa-rounded-full elsa-border elsa-border-transparent elsa-bg-white shadow elsa-text-gray-400 hover:elsa-text-blue-500 focus:elsa-text-blue-500 hover:elsa-ring-2 hover:elsa-ring-offset-2 hover:elsa-ring-blue-500 focus:elsa-outline-none focus:elsa-ring-2 focus:elsa-ring-offset-2 focus:elsa-ring-blue-500"
        onClick={(e) => this.onAddActivity(e)}
        style={{ right: '19rem' }}
      >
        <svg
          viewBox="0 0 1024 1024"
          ...
          fill="currentColor"
        >
          <path d="M1024 448H576V0H448v448H0v128h448v448h128V576h448z" p-id="9013"></path>
        </svg>
      </button>
    )
  }
  ...
  render() {
    return (
      <Host style={{ height: this.canvasHeight }}>
        <div
          class="workflow-canvas elsa-flex-1 elsa-flex"
          onDrop={(e) => this.onActivityDrop(e)}
          onDragOver={(e) => this.onActivityDragOver(e)}
        >
