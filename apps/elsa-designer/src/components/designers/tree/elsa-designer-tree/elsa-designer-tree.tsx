...
export class ElsaWorkflowDesigner {
  ...
  zoomParams: { x: number; y: number; scale: number; initialZoom: boolean } = {
    x: 0,
    y: 0,
    scale: 1,
    initialZoom: true
  ...
  @Watch('model')
  handleModelChanged(newValue: WorkflowModel, oldValue: WorkflowModel) {
  ...
  componentDidLoad() {
    ...
    this.safeRender()
    console.log('mode', WorkflowDesignerMode[this.mode])
  }
  ...
  async getActivityDisplayContext(activityModel: ActivityModel): Promise<ActivityDesignDisplayContext> {
    ...
    const descriptorExists = !!descriptor
    const oldContextData = (this.oldActivityDisplayContexts &&
      this.oldActivityDisplayContexts[activityModel.activityId]) || { expanded: false }
  ...
  updateWorkflowModel(model: WorkflowModel, emitEvent = true) {
    ...
    if (emitEvent) this.workflowChanged.emit(model)
  }
  ...
  applyInitialZoom() {
    ...
    this.svgD3Selected.call(
      this.zoom.transform,
      d3.zoomIdentity.scale(zoomParamsScale).translate(zoomParamsX, zoomParamsY)
    )
  ...
  setEntities() {
    ...
    // console.log('🚀 ~ file: elsa-designer-tree.tsx ~ line 603 ~ ElsaWorkflowDesigner ~ setEntities ~ rootActivities', rootActivities);

    // Connections between activities and their outcomes.

    // 2.连接activity-outcome: 设置(生成)activity的outcome(结果, 也可理解成执行分支)
    const activityDisplayContexts = this.activityDisplayContexts || {}
    this.workflowModel.activities.forEach((activity) => {
      // 生成activity html node
  ...
  onActivityPicked = async (args) => {
    ...
    this.addingActivity = true
    if (activityDescriptor.category === 'Run Workflow') {
      // 添加工作流直接update activity
      this.onUpdateActivity(activityModel)
    } else {
  ...
  renderNodes() {
    ...
    if (this.mode == WorkflowDesignerMode.Edit) {
      ...
      root.selectAll('.node.add').each((n: any) => {
        ...
        d3.select(node.elem)
          .on('click', async (e) => {
            ...
            e.preventDefault()
            root
              .selectAll('.node.add svg')
              .classed('elsa-text-green-400', false)
              .classed('elsa-text-gray-400', true)
              .classed('hover:elsa-text-blue-500', true)
            this.parentActivityId = node.activity.activityId // 设置父活动节点
            this.parentActivityOutcome = node.outcome // 父活动节点的运行结果
            // console.log('🚀 ~ file: elsa-designer-tree.tsx ~ line 719 ~ ElsaWorkflowDesigner ~ root.selectAll ~ e.shiftKey', e.shiftKey);
            if (e.shiftKey) {
              d3.select(node.elem)
                .select('svg')
                .classed('elsa-text-green-400', true)
                .classed('elsa-text-gray-400', false)
                .classed('hover:elsa-text-blue-500', false)
              return
            }
      ...
      // console.log('🚀 ~ file: elsa-designer-tree.tsx ~ line 700 ~ ElsaWorkflowDesigner ~ renderNodes ~ root', root.selectAll('.edgePath'));
  ...
  renderTree() {
    this.applyZoom() // 应用缩放
    this.setEntities() // 设置实体
    this.renderNodes() // 渲染节点
  }
  ...
  renderOutcomeButton() {
    const cssClass =
      this.mode == WorkflowDesignerMode.Edit ? 'hover:elsa-text-blue-500 elsa-cursor-pointer' : 'elsa-cursor-default'
  ...
  render() {
    console.log('eals-deginger-tree rerender')
      ...
      <Host class="workflow-canvas elsa-flex-1 elsa-flex" ref={(el) => (this.el = el)}>
        {this.mode == WorkflowDesignerMode.Test ? (
          <div>
            ...
            <div
              id="top"
              style={{ border: `4px solid orange`, position: `fixed`, height: `4px`, left: `0`, right: `0`, top: `30` }}
            />
            <div
              id="bottom"
              style={{
        ...
        <svg
          ref={(el: SVGSVGElement) => (this.svg = el)}
          id="svg"
          style={{
            height: this.canvasHeight,
            width: '100%',
