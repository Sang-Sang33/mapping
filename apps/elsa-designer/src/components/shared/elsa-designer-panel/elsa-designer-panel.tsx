              ...
              <div>
                <div class="elsa-relative elsa-flex elsa-items-start elsa-ml-1">
                  ...
                  <div class="elsa-ml-3 elsa-text-sm">
                    <label htmlFor={name} class="elsa-font-medium elsa-text-gray-700">
                      {t(`${name}Name`)}
                    </label>
                    <p class="elsa-text-gray-500">{t(`${name}Description`)}</p>
                  </div>
                </div>
                <div class="elsa-ml-1 elsa-my-4">{this.renderFeatureData(name, feature)}</div>
              </div>
            )
          })}
        </div>
      ...
      case featuresDataManager.supportedFeatures.workflowLayout:
        return (
          <select
            id={name}
            name={name}
            onChange={(e) => this.onPropertyChange(e, name)}
          ...
          >
            {Object.keys(LayoutDirection).map((key) => {
              return (
                <option value={LayoutDirection[key]} selected={LayoutDirection[key] === feature.value}>
                  {t(key)}
                </option>
              )
            })}
...
DashboardTunnel.injectProps(ElsaDesignerPanel, ['culture'])
